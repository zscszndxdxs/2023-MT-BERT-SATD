...
class BertForSequenceClassification(nn.Module):
    ...
    def __init__(self, config, num_labels):
        ...
        self.classifier_3 = nn.Linear(config.hidden_size, 2)
        self.classifier_4 = nn.Linear(config.hidden_size, 2)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, dataset_labels=None, task = None):
        _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask)
        pooled_output = self.dropout(pooled_output)
        #print("pooled_out================================================")
        #print("dataset_label=",dataset_labels[0].item())
        ...
        if str(task) == '5':
            logits1 = self.classifier_1(pooled_output)
            logits2 = self.classifier_2(pooled_output)            
            logits3 = self.classifier_3(pooled_output)            
            logits4 = self.classifier_4(pooled_output)
